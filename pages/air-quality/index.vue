<template>
  ...
  <div v-else>
    <div class="mt-5" v-if="!isDetail">
      <v-list class="py-0">
        <v-list-item
          v-for="air_quality in air_qualitys || []"
          :key="air_quality.id"
          @click="setPageData(air_quality)"
          class="px-3"
        >
          <v-list-item-avatar
    ...
    <div class="mx-n2" v-else>
      ...
      <div class="px-4">
        <v-row no-gutters class="my-3">
          <v-col cols="2">
            <p
              class="mb-0 text-center title1--text secondary--text text-no-wrap"
            >
              Элемент
            </p>
          </v-col>
...
export default {
  ...
  methods: {
    setPageData(el) {
      this.quality_data = el;
      this.isDetail = !this.isDetail;
      // this.$store.commit("setPageData", el);
      // this.$router.push(`air-quality/${el.id}`);
    },
  },
  async fetch() {
    console.log(this.loading);
    this.loading = true;
    this.air_qualitys = await fetch(
      "http://agaar.mn:8080/aqdb/api/station/list"
    )
      .then((res) => res.json())
